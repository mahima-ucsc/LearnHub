...
<div id="adPreviewModal" class="post-preview-modal">
    <div class="modal-content">
        ...
        <div class="modal-actions">
            <button class="btn btn-remove btn-text" id="removeAdBtn">
...
<script>
    ...
    function renderAdvertisements(advertisements) {
        ...
        if (advertisements.length === 0) {
            tableBody.innerHTML = '<tr><td colspan="5" style="text-align: center;">No advertisements to show</td></tr>';
            return;
        }
        ...
        advertisements.forEach(ad => {
            ...
            row.setAttribute('data-user-id', ad.user_id);
            ...
            row.innerHTML = `
                ...
                <td>
                    <div class="status ${(ad.status).toLowerCase()}">${ad.status}</div>
                </td>
                <td>${ad.package}</td>
                <td>Rs. ${ad.price || '0'}</td>
                <td>
                    <div class="action-buttons">
                        ${ad.status === 'approved' ? '<button class="btn btn-sm btn-remove"><i class="fas fa-times"></i></button>' : ''}
                        <button class="btn btn-sm btn-preview"><i class="fas fa-eye"></i></button>
                    </div>
                </td>
            `;
    ...
    // Function to handle advertisement removal
    function handleRemoveAd(adId) {
        const row = document.querySelector(`tr[data-ad-id="${adId}"]`);
        const endpoint = `/remove-advertisement/${adId}`;
            ...
            .then(data => {
                if (data.success) {
                    showToast(
                        ...
                        'The advertisement has been successfully removed from the system.',
                        'success'
                    );
                    ...
                    showToast('Error', data.message || 'Failed to remove advertisement.', 'error');
                }
            })
            .catch(error => {
                console.error('Error:', error);
                showToast('Error', 'There was a problem removing the advertisement. Please try again.', 'error');
    ...
    document.addEventListener('DOMContentLoaded', () => {
        if (typeof advertisementData !== 'undefined' && advertisementData.length > 0) {
            ...
            const tableBody = document.querySelector('.posts-table tbody');
            tableBody.innerHTML = '<tr><td colspan="5" style="text-align: center;">No advertisements data available</td></tr>';
        }
    });
    ...
    function updateStatistics() {
        ...
        document.querySelector('.post-stat-card:nth-child(3) .value').textContent = rejectedCount;
    }
</script>
